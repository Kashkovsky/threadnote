        ...
        body: [
          ...
          },
          {
            ...
            items: [
              ...
              'For ordinary model-facing Context Brief and local repository graph reads, prefer responseFormat=agent; named Worksets use dual or text until their agent projection exists. Inspect the advertised schema for older-server compatibility.',
...
# Existing output files are never overwritten.`,
          ...
          {
            type: 'paragraph',
            text: 'For ordinary local repository model reads, inspect_code_graph responseFormat=agent returns a schema-aware, text-only TN-GRAPH/1 projection; budgetTokens is applied after final formatting and semantic truncation. Named Worksets currently use responseFormat=text for lossless JSON in one text block or the default responseFormat=dual for readable text plus structured compatibility. Inspect the advertised schema and fall back on older servers. Indexing, timeout, and error responses keep their existing status format.',
          },
        ],
      },
      {
