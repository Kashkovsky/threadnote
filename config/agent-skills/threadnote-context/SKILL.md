...
managed-memory URI in `memoryRefs` on the same recall call. Optionally narrow the one-hop expansion with
`relationTypes`: `depends_on`, `evidence_for`, `references`, `related_to`, or `supersedes`. This is explicit navigation,
not recursive graph discovery. Use the returned premise and connection receipts to understand resolution, currentness,
direction, and truncation; read only useful neighbor pointers before relying on them. An unresolved, conflicted, or
out-of-scope premise does not expand, and truncated connection coverage is not evidence that other neighbors are absent.
