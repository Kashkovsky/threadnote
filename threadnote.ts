...
const USER_INSTRUCTIONS_START_MARKER = '<!-- BEGIN THREADNOTE USER INSTRUCTIONS -->';
const USER_INSTRUCTIONS_END_MARKER = '<!-- END THREADNOTE USER INSTRUCTIONS -->';
  ...
  return Promise.all(
    USER_AGENT_INSTRUCTION_TARGETS.map(async target => {
      ...
      if (existingBlock === undefined) {
        return {name: target.label, status: 'warn', detail: `${targetPath} missing threadnote block; install will add it`};
      }
      if (existingBlock !== expectedBlock) {
        return {name: target.label, status: 'warn', detail: `${targetPath} has stale threadnote block; install will update it`};
      }
