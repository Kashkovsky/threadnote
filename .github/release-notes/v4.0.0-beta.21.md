...
Threadnote 4 beta.21 removes the aggregate source-byte ceiling from the native code graph so large monorepos can be
...
```bash
curl -fsSL https://raw.githubusercontent.com/Kashkovsky/threadnote/v4.0.0-beta.21/scripts/install.sh |
  sh -s -- --beta
```
