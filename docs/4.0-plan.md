...
`data/viking/<account>` directory is a migration source only. Safe migration may leave empty directories and ignored
OS/runtime metadata there; they never regain canonical authority, and later material writes are reported rather than
recursively deleted.
