...
1. remove blocking whole-worktree observation from ordinary immutable-snapshot reads and establish an exact-HEAD
   benchmark contract;
2. introduce query-stage telemetry through a gateway-first schema-v4 and consent-v4 rollout;
...
### Slice 2 — telemetry schema v4: foundation implemented; rollout gated

The immutable schema-v4 gateway/Collector contract, eight-trace/four-version storage canary, consent-v4 producer, and
generated Grafana views are implemented locally. Schema v3 remains the frozen automatic-update contract from main; schema v4
preserves that surface and adds graph-query observations. The dashboard admits v1/v2/v3/v4 generic telemetry,
v2/v3/v4 graph builds, v3/v4 automatic updates, and v4 graph queries; every Tempo query excludes the exact synthetic
canary version, and its unexpected-full percentage is zero-denominator safe. Query telemetry separates status,
snapshot selection/refresh, and execution and adds only closed request/scope/selection/freshness values plus published
file/symbol/edge-count buckets. A first bounded follow-up now attributes repository-identity resolution, worktree
observation, final strict reobservation, and response serialization. An absent subphase means the stage executed
normally; `skipped` and `fallback` are explicit, closed exceptional subphases. Analyze requests now emit the same status
and snapshot coverage as inspect requests.

Deployment remains a hard gate: gateway/Collector first, then all eight stored v1-v4 canary traces and the dashboard, and
only then the consent-v4 producer. This foundation does not yet close the broader P0.3 acceptance criteria:
dirty-overlay construction, candidate selection, hydration, traversal, database-open attribution, and the remaining
bounded result classifications remain follow-up work before Slice 2 is declared complete.
...
- Telemetry schemas are immutable. Query-stage and repository-shape fields require schema v4, consent v4, and a
  gateway-first rollout; they are never appended to frozen schema v2 or automatic-update schema v3.
...
### P0.3 / Slice 2 — gateway-first telemetry schema v4

Schemas v1, v2, and v3 are immutable allowlists. Schema v2 covers terminal graph-build classifications and schema v3
adds the closed automatic-update result, but neither admits query-stage or repository-shape observations. Define query
observability as schema v4 and treat it as a material data category that invalidates consent v3. Preserve the
traces-only isolation in [telemetry.md](telemetry.md): do not enable the application's existing Effect span tree for
export. A dedicated reporter emits only the closed v4 stage surface.

The schema-v4 rollout order is a hard compatibility gate:

1. Specify the closed v4 resource/span envelope, field combinations, byte/rate budget, privacy review, documentation,
   and consent-v4 preview text.
2. Deploy a gateway/Collector revision that admits frozen v1, v2, and v3 plus candidate v4. Do not release a v4
   producer yet.
3. Extend the synthetic storage canary and require all supported versions to be accepted, stored, and queryable.
4. Deploy v4 dashboard panels and verify their queries against synthetic v4 data.
5. Release the producer with `TELEMETRY_CONSENT_VERSION = 4`. Existing v1/v2/v3 opt-ins fail closed until the user
   reviews and reapplies consent; Threadnote never migrates consent silently.
6. Keep v1/v2/v3 ingress and canaries until their supported producers are retired in a separate reviewed change.

This preserves the gateway-first compatibility rule in the
[production telemetry runbook](operations/telemetry-production.md); a v4 canary rejection blocks the producer release
even when the health endpoint and older canaries pass.

Schema v4 measures these nested graph-inspection stages independently:
...
not unbounded label dimensions. If anonymous telemetry is disabled or consent is older than v4, query telemetry remains
...
- frozen schema-v1/v2/v3 fixtures and producers remain byte-for-byte compatible;
- the v4 gateway rejects query fields on v1/v2/v3, unknown fields, invalid combinations, and over-budget envelopes;
- every v4 graph operation produces one terminal outcome and balanced stage spans;
...
- consent v3 fails closed after the producer requires consent v4, and preview/apply is explicit;
...
- P0 exact-HEAD benchmark and deterministic incremental/full parity fixture; production expansion additionally uses
  the schema-v4 stage telemetry cohort when available;
...
| Schema-v4 query telemetry        | Gateway-first v4 allowlist/canary plus explicit consent v4                     | Production cohort rollout decisions    |
...
1. Land exact-HEAD benchmark and local measurement controls before behavior changes. External query telemetry follows
   the separately gated schema-v4/consent-v4 rollout and is required before broad production expansion, not before the
...
- 2026-08-20: broad source, literature, algorithm, benchmark, and anonymous Grafana/Tempo assessment completed on
  ...
  Slice 1. Review confirmed that query-stage fields cannot extend a frozen telemetry schema/consent contract, so the
  plan moved gateway-first query telemetry to Slice 2 and the qualified watcher receipt to Slice 3. After main assigned
  schema/consent v3 to automatic-update results, the graph-query surface moved intact to schema/consent v4. Slice 1
...
- 2026-08-20: Slice 2's schema-v4 foundation was implemented in draft PR #174: immutable
  v1/v2/v3-compatible gateway admission,
  Collector projection, eight-trace/four-version canary coverage, consent-v4 fail-closed producer behavior, privacy-safe query
  lifecycle checkpoints, and generated Grafana query panels. Dashboard regressions were closed by excluding
  `0.0.0-canary` from all Tempo targets and guarding the unexpected-full denominator. Focused producer/gateway/MCP
  integration and property tests pass. Production rollout and the finer stage surface remain gated work.
- 2026-08-20: the first bounded schema-v4 query-stage follow-up added snapshot-free checkpoints for repository identity,
  worktree observation, strict final reobservation, and response serialization. It distinguishes a deliberate skip from
