...
Status: approved; Slice 1 implemented; Slice 2 schema-v3 foundation and first bounded stage slice implemented locally; rollout gated
...
### Slice 2 — telemetry schema v3: foundation implemented; rollout gated

The immutable schema-v3 gateway/Collector contract, four-trace storage canary, consent-v3 producer, and generated
Grafana views are implemented locally. The dashboard admits v1/v2/v3 generic telemetry, v2/v3 graph builds, and v3
graph queries; every Tempo query excludes the exact synthetic canary version, and its unexpected-full percentage is
zero-denominator safe. Query telemetry separates status, snapshot selection/refresh, and execution and adds only closed
request/scope/selection/freshness values plus published file/symbol/edge-count buckets. A first bounded follow-up now
attributes repository-identity resolution, worktree observation, final strict reobservation, and response serialization.
An absent subphase means the stage executed normally; `skipped` and `fallback` are explicit, closed exceptional
subphases. Analyze requests now emit the same status and snapshot coverage as inspect requests.

Deployment remains a hard gate: gateway/Collector first, then all four stored canary traces and the dashboard, and
only then the consent-v3 producer. This foundation does not yet close the broader P0.3 acceptance criteria: dirty-overlay
construction, candidate selection, hydration, traversal, database-open attribution, and the remaining bounded result
classifications remain follow-up work before Slice 2 is declared complete.
...
- 2026-08-20: Slice 2's schema-v3 foundation was implemented locally: immutable v1/v2-compatible gateway admission,
  Collector projection, four-shape canary coverage, consent-v3 fail-closed producer behavior, privacy-safe query
...
- 2026-08-20: the first bounded schema-v3 query-stage follow-up added snapshot-free checkpoints for repository identity,
  ...
  database-read attribution remain pending; schemas v1 and v2 are unchanged.
