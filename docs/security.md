...
extraction. macOS additionally requires valid code signatures and notarization assessment; Windows requires valid
Authenticode signatures for the executable and native payload. Linux relies on GitHub's immutable-release attestation
as the publisher trust root. The repository's release authority is intentionally trusted: this model detects corrupted,
substituted, mutable, or unsigned delivery, but it does not claim to survive compromise of the repository publisher
itself. Custom release APIs require an explicit untrusted-source opt-in.
