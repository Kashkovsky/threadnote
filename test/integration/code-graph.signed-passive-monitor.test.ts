...
describe('passive signed graph delivery monitor', () => {
  effectIt.effect('replays the exact prepared operation after a coordinator outage and monitor restart', () =>
    TestClock.withLive(
      Effect.gen(function* () {
        ...
        const services = <A, E, R>(effect: Effect.Effect<A, E, R>) => effect.pipe(
        ...
        yield* waitForEffect(listGraphWorkerDeliveryOutboxOperations(home, scope).pipe(Effect.map(items => items.length === 0)));
