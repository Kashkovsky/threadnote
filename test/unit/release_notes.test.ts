...
describe('fetchThreadnoteReleaseNotes', () => {
  effectIt.effect('includes GitHub prereleases only when the beta channel requests them', () => Effect.gen(function* () {
    vi.stubGlobal(
      'fetch',
      vi.fn(async () =>
        Response.json([
    ...
    const beta = yield* (fetchThreadnoteReleaseNotes({includePrereleases: true}).pipe(provideTestLayer(ApplicationLayer)));
