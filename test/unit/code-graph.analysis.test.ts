...
describe('code graph analysis', () => {
  it('computes deterministic statistics, components, communities, and cross-community links', async () => {
    ...
    const first = await Effect.runPromise(analyzeCodeGraph(pagedAnalysisStore(symbols, edges), options));
    const second = await Effect.runPromise(
      analyzeCodeGraph(pagedAnalysisStore([...symbols].reverse(), [...edges].reverse()), {
        ...options,
        budget: {pageSize: 2},
      }),
    );
  ...
  it('detects statistically exceptional god nodes without treating every high-degree node as one', async () => {
    const center = analysisSymbol('router', '@acme/router', 'packages/router/src/router.ts');
    const leaves = Array.from({length: 12}, (_, index) =>
      analysisSymbol(`handler-${index.toString().padStart(2, '0')}`, `@acme/handler-${index}`, `handlers/${index}.ts`),
    );
    const symbols = [center, ...leaves];
    const edges = leaves.map((leaf, index) => analysisEdge(`route-${index.toString().padStart(2, '0')}`, center, leaf));
  ...
  it('audits numeric confidence and provenance in the topology pass with bounded deterministic findings', async () => {
    ...
    const result = await Effect.runPromise(
      analyzeCodeGraph(pagedAnalysisStore([source, target], edges), {
  ...
  it('keeps one strongest surprising link per community boundary', async () => {
  ...
  it('drills into a stable community ID with bounded and honest member coverage', async () => {
  ...
  it('derives stable bounded n-ary relationship groups from high-degree fan patterns', async () => {
  ...
  it('leases the ready snapshot and releases it after success and interruption-safe failure', async () => {
    ...
    await Effect.runPromise(
      analyzeCodeGraphWithLease(successful, {
        databasePath: '/analysis/graph.sqlite',
        snapshot,
      }),
    );
    ...
    const failed = leasedAnalysisStore(
      {
        ...
        loadSymbolPage: () => Effect.fail(new Error('read failed')),
      } as unknown as CodeGraphStoreShape,
      failedEvents,
    );
    await expect(
      Effect.runPromise(
        analyzeCodeGraphWithLease(failed, {
          databasePath: '/analysis/graph.sqlite',
          snapshot,
        }),
      ),
  ...
  it('counts each symbol identity once even if a malformed page repeats a row', async () => {
    ...
    const result = await Effect.runPromise(
      analyzeCodeGraph(pagedAnalysisStore([first, first, second], []), {
        budget: {pageSize: 3},
        databasePath: '/analysis/graph.sqlite',
        snapshot,
      }),
  ...
  it('derives omitted graph-size budgets from the snapshot without fixed admission caps', async () => {
    const result = await Effect.runPromise(
      analyzeCodeGraph(pagedAnalysisStore([], []), {
        databasePath: '/analysis/graph.sqlite',
        snapshot: {
          ...analysisSnapshot([], []),
  ...
  it('returns an honest partial result when node, edge, visit, and output limits are exhausted', async () => {
  ...
  it('remains cooperatively cancellable while a page read is pending', async () => {
    await Effect.runPromise(
      Effect.gen(function* () {
...
function projectStableResult(result: Awaited<ReturnType<typeof runFixtureAnalysis>>) {
  return {
