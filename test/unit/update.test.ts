...
describe('runUpdate', () => {
  ...
  it.runIf(process.platform !== 'win32')(
    'rewrites a managed stale launcher after an npm update even when repair is disabled',
    async () => {
      const config = await makeRuntime();
      homes.push(config.agentContextHome);
      vi.mocked(utils.currentPackageVersion).mockReturnValue(Effect.succeed('4.0.0-beta.5'));
      mockRegistryVersions('4.0.0', '4.0.0-beta.6');
      const userHome = join(config.agentContextHome, 'user');
      const prefix = join(config.agentContextHome, 'current-node');
      const packageRoot = join(prefix, 'lib', 'node_modules', 'threadnote');
      const packageEntry = join(packageRoot, 'bin', 'threadnote.cjs');
      const launcher = runtimeThreadnoteBinPath('npm', prefix, process.platform);
      ...
      await writeFile(join(packageRoot, 'package.json'), '{"name":"threadnote","version":"4.0.0-beta.6"}\n');
      await writeFile(packageEntry, '#!/usr/bin/env node\n');
      await chmod(packageEntry, 0o755);
      await symlink('../lib/node_modules/threadnote/bin/threadnote.cjs', launcher);
      await writeFile(
        commandShim,
        [
          '#!/usr/bin/env bash',
      ...
      const testSystem = SystemInfo.of({
        ...
        environment: () => ({
          ...
          NVM_DIR: undefined,
          NVM_HOME: undefined,
          THREADNOTE_BIN_DIR: dirname(commandShim),
        }),
      ...
      expect(result.output).toContain(`Wrote command shim: ${commandShim}`);
      expect(await readFile(commandShim, 'utf8')).toContain('/current-node/lib/node_modules/threadnote');
      expect(await readFile(commandShim, 'utf8')).not.toContain('/stale/nvm/versions/node/v22.21.1');
      expect(executeStreaming).not.toHaveBeenCalledWith(launcher, ['repair', '--no-post-update'], expect.anything());
    },
  );
