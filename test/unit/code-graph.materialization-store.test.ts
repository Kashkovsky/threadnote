...
describe('code graph full-build materialization store', () => {
  ...
  effectIt.effect('invalidates retained lookup summaries when a resolved reference publishes an alias', () =>
    Effect.gen(function* () {
      ...
      const aliasKey = 'typescript:name:lookupCacheAlias';
      const target = symbol('lookup-cache-target', 'lookupCacheTarget', [targetKey]);
      const producer = symbol('lookup-cache-producer', 'lookupCacheProducer', ['typescript:name:lookupCacheProducer']);
      const consumerA = symbol('lookup-cache-consumer-a', 'lookupCacheConsumerA', [
        'typescript:name:lookupCacheConsumerA',
      ]);
      const consumerB = symbol('lookup-cache-consumer-b', 'lookupCacheConsumerB', [
        'typescript:name:lookupCacheConsumerB',
      ]);
      const producerEdge = edge('lookup-cache-producer-edge', producer, target.name);
      const consumerEdgeA = edge('lookup-cache-consumer-edge-a', consumerA, target.name);
      const consumerEdgeB = edge('lookup-cache-consumer-edge-b', consumerB, target.name);
      const reference = (
        unresolved: CodeGraphEdge,
        source: CodeGraphSymbol,
        lookupKey: string,
        aliasLookupKeys?: readonly string[],
      ): CodeGraphReference => ({
        ...(aliasLookupKeys === undefined ? {} : {aliasLookupKeys}),
        edgeId: unresolved.id,
      ...
      const result = yield* store.withSession(
        ...
        Effect.gen(function* () {
          ...
          yield* store.stageActivationFacts(
            fixture.databasePath,
            [target, producer, consumerA, consumerB],
            [consumerEdgeA, consumerEdgeB, producerEdge],
            [
              reference(consumerEdgeA, consumerA, aliasKey),
              reference(consumerEdgeB, consumerB, aliasKey),
              reference(producerEdge, producer, targetKey, [aliasKey]),
            ],
            undefined,
            0,
          );
      ...
      expect(result.resolution).toMatchObject({aliasesDiscovered: 1, passesCompleted: 2, resolved: 3});
      expect(result.graph.edges.map(value => value.targetId)).toEqual([target.id, target.id, target.id]);
    }).pipe(provideTestLayer(ApplicationLayer)),
  );
