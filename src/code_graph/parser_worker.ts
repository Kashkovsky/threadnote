                        ...
                        capacity === 1
                          ? (() => {
                              ...
                              const extract = (file: CodeGraphInventoryFile) => {
                                if (warmFailure !== undefined) return Effect.succeed(degradedResult(file, warmFailure));
                                if (warmed) return extractFromSlot(slot, file, threadnoteHome);
                                return fromPromiseInterruptibleAwaiting(
                                  signal => slot.warm(threadnoteHome, signal),
                                  cause =>
                                    Schema.is(ParserWorkerError)(cause) ? cause : ParserWorkerError.of('protocol'),
                                ).pipe(
                                  Effect.tap(() => Effect.sync(() => (warmed = true))),
                                  Effect.andThen(extractFromSlot(slot, file, threadnoteHome)),
                                  Effect.catch(error =>
                                    Effect.sync(() => {
                                      warmFailure = error;
                                      return degradedResult(file, error);
                                    }),
                                  ),
                                );
                              };
                              return use(extract).pipe(
