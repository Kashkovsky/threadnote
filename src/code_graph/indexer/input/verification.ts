...
import {verifyIndexInput} from '../materialization.js';
import {WorktreeChangedDuringIndex} from '../shared.js';
...
export const verifyCommittedIndexInput = Effect.fn('codeGraph.verifyCommittedIndexInput')(function* (input: {
  ...
  readonly scopeInventory?: CodeGraphInventory;
  ...
  return yield* verifyIndexInput(
    input.identity,
    true,
    input.threadnoteHome,
    input.requestedOverlay,
    input.scopeInventory,
  ).pipe(
    Effect.catchIf(
