...
export const CODE_MEMORY_LINK_DOGFOOD_VERSION = 1 as const;
...
export function parseCodeMemoryLinkDogfoodArtifactV1(value: unknown): CodeMemoryLinkDogfoodArtifactV1 {
  ...
  exactKeys(
    artifact,
    ['artifactHash', 'candidate', 'harnessCommit', 'observations', 'runId', 'version'],
    'dogfood artifact',
  );
...
export function evaluateCodeMemoryLinkDogfood(value: unknown): CodeMemoryLinkDogfoodResultV1 {
  ...
  const qualityFailures = qualityGateFailures(artifact.observations);
...
function normalizeArtifact(value: unknown): Omit<CodeMemoryLinkDogfoodArtifactV1, 'artifactHash'> {
  const artifact = record(value, 'dogfood artifact');
  exactKeys(artifact, ['candidate', 'harnessCommit', 'observations', 'runId', 'version'], 'dogfood artifact evidence');
  if (artifact.version !== CODE_MEMORY_LINK_DOGFOOD_VERSION) invalid('dogfood artifact version must be 1');
  ...
  return {
    candidate,
    harnessCommit,
    observations: parseObservations(artifact.observations, candidate, harnessCommit, runId),
    runId,
    version: CODE_MEMORY_LINK_DOGFOOD_VERSION,
  };
}
...
function parseObservations(
  ...
  assertUniqueCodeMemoryLinkAttestations(
    observations.map(observation => observation.attestation),
...
function qualityGateFailures(observations: readonly CodeMemoryLinkDogfoodObservationV1[]): string[] {
