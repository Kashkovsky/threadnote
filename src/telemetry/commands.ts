...
const TELEMETRY_DATA_SUMMARY =
  'General CLI and MCP operations, event timestamps and durations, outcomes, bounded runtime memory/resource observations, phases/state, safe typed failures, app/runtime/schema versions, platform, architecture, and random session/invocation/OTLP transport IDs. Automatic-update worker completions additionally include one closed automatic-update result (busy, current, disabled, failed, or updated); updated results include a repair-required boolean, while failed results retain the bounded failure outcome/type. Successful graph builds additionally include coarse build-kind/materialization/fallback/closure/efficiency classifications, changed/deleted/delta/extracted/reused/staged/total file-count buckets, cached/changed/final fact-byte buckets, and rewrite/replay-amplification buckets. Failed graph builds add only bounded outcome/type to that lifecycle surface; interrupted graph builds add only outcome/duration. Neither adds graph classifications or buckets. MCP graph inspection diagnostics additionally include closed request-kind, local/workset scope, and status/snapshot/execute stage labels. A selected local published snapshot adds only active/promoted/borrowed selection, current/deferred/stale freshness, and coarse file/symbol/edge-count buckets. Successful Context Brief diagnostics additionally include closed local/workset scope, task-only-v2/code-anchored-v3 contract, brief/locate/explain/trace/impact mode, graph/memory/code-linked-memory/citation-validation/projection phases, returned graph/memory lane and bounded gap classes, code-anchor coverage plus recovery-present state and power-of-two requested/resolved/matched-memory buckets, none/complete/partial/unavailable validation coverage, none/exact-only/relocated/stale/unknown/mixed citation result, a closed unknown-reason class when applicable, output-truncated state, and power-of-two buckets for cited memories, citations by status, repositories validated, and cache hits. Deferred code-anchor finalization checkpoints additionally include a closed trigger/result and power-of-two latency and work-count buckets. Non-successful Context Brief and finalization checkpoints never include result-derived fields.';
const TELEMETRY_EXCLUSION_SUMMARY =
  'Never arguments, environment values, memory or prompt content or identity, task/query text, recall queries/results, MCP payloads/results, paths, node/symbol/citation identity, repository/workset/commit/snapshot/hash identity, exact private counts, exception messages/stacks, user, account, or agent IDs.';
  ...
  const endpoint = yield* Effect.try({
    try: () => normalizeTelemetryEndpoint(options.endpoint ?? renewal?.endpoint ?? DEFAULT_TELEMETRY_ENDPOINT),
      ...
      Schema.is(TelemetryConfigurationError)(cause)
        ? cause
        : TelemetryConfigurationError.make({message: 'Telemetry endpoint validation failed.', cause}),
  });
