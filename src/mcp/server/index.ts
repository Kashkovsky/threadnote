      ...
      Effect.gen(function* () {
        ...
        const instructions = memoryScope
          ...
          : toolset === CURSOR_CLOUD_LOCAL_MCP_TOOLSET
            ...
            : 'For non-trivial local repo work, call MCP `context_brief` with task + absolute `callerCwd`. Prefer `responseFormat: "agent"`. `threadnote context brief --cwd <cwd> --task <task>`. `recall_context` + `read_context`: memory alternative; `threadnote://` pointers unread, not evidence. Use `inspect_code_graph`/`analyze_code_graph`, then exact source. Close with private `remember_context(kind=handoff)`. Optional five-field KD: `review_session_context`; `apply_memory_candidates` with `approve` (optional `editedText`), `defer`, or `reject`. Never auto-apply/share. No sensitive data; confirm publishes; never publish handoffs/preferences.';
