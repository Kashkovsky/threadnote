...
export function buildOnboardingGuide(state: OnboardingState): string {
  ...
  return [
    ...
    '',
    'Capture work — at meaningful task closeout, check decisions, invariants, preferences, and',
    'handoff state against current memory, then show the user at most three recommendations.',
    '  Run: review_session_context({"task":"...","outcome":"...","project":"<repo>","topic":"<feature>",',
    '  "decisions":["..."],"handoff":["..."]}), then apply_memory_candidates only after the user',
    '  explicitly approves, edits, defers, or rejects each candidate.',
    '  For a direct “remember this” request: remember_context({"text":"...","project":"<repo>","topic":"<feature>"}).',
    '',
